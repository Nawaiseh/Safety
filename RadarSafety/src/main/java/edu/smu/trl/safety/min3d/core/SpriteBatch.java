...
public class SpriteBatch {
    ...
    //--Members--//
    GL10 gl;                                           // GL Instance
    TextVertices TextVertices;                                 // Vertices Instance Used for Rendering
    ...
    public SpriteBatch(GL10 gl, int maxSprites) {
        ...
        this.TextVertices = new TextVertices(gl, maxSprites * VERTICES_PER_SPRITE, maxSprites * INDICES_PER_SPRITE, false, true, false);  // Create Rendering Vertices
        ...
        TextVertices.setIndices(indices, 0, len);         // Set Index Buffer for Rendering
    }
    ...
    public void endBatch() {
        if (numSprites > 0) {                        // IF Any Sprites to Render
            TextVertices.setVertices(vertexBuffer, 0, bufferIndex);  // Set Vertices from Buffer
            TextVertices.bind();                             // Bind Vertices
            TextVertices.draw(GL10.GL_TRIANGLES, 0, numSprites * INDICES_PER_SPRITE);  // Render Batched Sprites
            TextVertices.unbind();                           // Unbind Vertices
        }
    }

    //--Draw Sprite to Batch--//
    // D: batch specified sprite to batch. adds TextVertices for sprite to vertex buffer
