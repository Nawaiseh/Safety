            ...
            synchronized (Lock) {
                if ((BluetoothChatService.AlertIsGoing) && ((TimeNow - BluetoothChatService.LastUpdate) > 3000)) {
                    BluetoothChatService.MessageHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            BluetoothChatService.Renderer_Activity.StopColorAnimation(BluetoothChatService.Renderer_Activity.Layout, BluetoothChatService.Renderer_Activity.ValueAnimator, BluetoothChatService.Renderer_Activity.NormalColor);
                        }
                    });
